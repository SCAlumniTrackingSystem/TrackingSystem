...
function send_password_reset ($get_email,$get_fname,$token){
   ...
   <h5>You receive this email because you are requesting for reseting your password for your account</h5>
   <br><br>
   <a href = 'http://localhost/alumnitracer/admin/a_password-change.php?token=$token&email=$get_email'> Click Me </a>
   ";
...
if ($update_token_run) {
    ...
    header("Location: a_password-reset.php");
    exit (0); 
}
else {
...
if (isset($_POST['password_update'])) {
        ...
        if (!empty($email) && !empty($new_password) && !empty($confirm_password)) {
            ...
            if (mysqli_num_rows($check_token_run) > 0) {
                ...
                if ($new_password == $confirm_password) {
                    ...
                    $update_pass = "UPDATE admin SET password = '$new_password' WHERE verify_token='$token' LIMIT 1";
                    ...
                    if ($update_pass_run) {
                        ...
                        $_SESSION['status_code'] = "error";
                        header("Location: a_password-change.php?token=$token&email=$email");
                        exit (0); 
    ...
    exit (0); 
        ...
        $_SESSION['status_code'] = "warning";
    header("Location: a_password-change.php");
    exit (0); 
    }
}
