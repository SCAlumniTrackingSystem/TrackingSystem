...
$query = "SELECT  * , CONCAT (fname, ' ' , midint ,' ',lname) AS name FROM request
INNER JOIN department on request.deptID = department.deptID
INNER JOIN courses on request.courseID = courses.courseID 
INNER JOIN batch on request.batchID = batch.batchID 
ORDER by ID asc ";
...
<div class="card mx-auto" style="width: 90%; margin-top: 12%;">
 <div class="card-header text-center">
    <b>LIST OF REQUESTING TO IMPORT/INSERT </b>
        ...
        while($row = mysqli_fetch_array($query_run)){
            ...
            echo '
                <tr>
                    ...
                    <td class="alumID" style="display:none"> '.$row["ID"].'</td>
                    <td> '.$row["name"].'</td>
                     <td> '.$row["dept"].'</td>
                    <td> '.$row["course"].'</td>
