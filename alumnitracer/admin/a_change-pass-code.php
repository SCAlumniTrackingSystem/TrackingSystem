...
if(isset($_POST['change']))
  ...
  $chg_pwd = mysqli_query($con, "SELECT * FROM admin WHERE adminID = '$id' LIMIT 1");
  $chg_pwd1=mysqli_fetch_array($chg_pwd);
  $data_pwd=$chg_pwd1['password'];
  if($data_pwd==$old_pass){
  if($new_pass==$re_pass){
     $new_pass = MD5 ($new_pass);
    $update_pwd=mysqli_query($con, "UPDATE admin SET password ='$new_pass' WHERE adminID = '$id'");
    
    $_SESSION['status'] = "Update Sucessfully";
    $_SESSION['status_code'] = "success";
            header("Location: a_change-pass.php");
            exit (0); 
  }
  else{
